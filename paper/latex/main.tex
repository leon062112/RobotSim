...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{figures/precision_tradeoff.pdf}
  \caption{Precision--performance trade-off over the complete 166,667-step trajectory. Bars report fused-kernel throughput on the left axis; markers report normalized absolute X- and Z-RMSE deviations from fused fp64 on the right axis. Each reduced-precision configuration changes only the named component, with all remaining operations in fp32/IEEE.}
  \label{fig:precision-tradeoff}
\end{figure}

\Cref{fig:precision-tradeoff} separates this performance effect from numerical deviation. Replacing fused fp64 arithmetic with fp32 increases throughput from 17.1 to 249.6 thousand steps/s, a $14.6\times$ improvement, while the normalized deviations remain approximately $1.1\times10^{-4}$ in X and $1.0\times10^{-5}$ in Z. Because the markers measure distance from the fp64 trajectory rather than error against ground truth, the plot does not imply that fp32 is intrinsically more accurate than fp64; fp64 is the zero-deviation numerical reference.

The component experiments show why precision must be assigned by numerical role. Storing only sensor input in fp16 preserves fp32-level throughput and produces an almost unchanged X RMSE, although the 2.19-mm Z-RMSE change remains visible relative to the much smaller fp64 Z error. TF32 dot products reduce throughput to 208.1 thousand steps/s and introduce additional deviation, demonstrating that Tensor Core setup and conversion are not amortized by these small matrices. Storing recurrent state in fp16 yields only a marginal throughput change but increases the normalized X deviation to approximately $2.8\times10^{2}$, indicating long-horizon divergence. Lower nominal precision therefore neither guarantees higher throughput nor preserves recursive-filter accuracy, motivating the component-aware scheme in \Cref{subsec:mixed-precision}.
