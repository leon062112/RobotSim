...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    \midrule
    PyTorch & \multicolumn{2}{c}{2.11.0} \\
    CUDA & \multicolumn{2}{c}{12.9} \\
    Triton & \multicolumn{2}{c}{3.6.0} \\
...
The precision study compares complete output trajectories rather than isolated filter steps: every method processes the same $166{,}667$-sample trajectory, and each result is scored by the per-axis position RMSE against the ground-truth trajectory and by the maximum axis-wise deviation from the fp64 CPU reference. Short shared-prefix checks are insufficient here, because \Cref{subsec:motivation-precision} shows that rounding error in recurrent quantities accumulates over the full horizon, which a truncated run would understate.

\Cref{tab:precision-fidelity} compares the numerical accuracy and throughput of the tensor baselines and of \textsc{Trident} under different precision plans on the H20 device. The eager and \texttt{torch.compile} implementations evaluate the same expressions as the reference, and their trajectories agree with it to the last printed digit on their shared prefix; full-length timing and accuracy runs for these two baselines are pending for the final manuscript. \textsc{Trident} in fp64 reproduces the reference to within $0.13$\,mm on every axis, and the adopted fp32 plan deviates by at most $0.13$\,mm---more than four orders of magnitude below the $1{,}017.8$\,mm reference positioning error itself. The accelerated implementations are therefore not less accurate than the tensor baselines: every configuration listed as safe matches the reference at or below the filter's own error scale while running up to two orders of magnitude faster. On the A100 device the same comparison holds with an even smaller TF32 deviation ($0.63$\,mm versus $4.71$\,mm), while the fp64 and fp32 throughputs differ from the values in the table.

The reduced-precision plans document where this equivalence breaks. Lowering only the sensor input to fp16 adds at most $2.19$\,mm and, as \Cref{tab:precision-isolation} shows, remains within the acceptance band, but it does not shorten the critical path and is therefore not adopted. Uniform reductions across the whole estimator, in contrast, are unsafe: bfloat16 sensor input inflates the vertical RMSE from $8.14$ to $1{,}014.1$\,mm, and fp16 storage of the recurrent state diverges over the horizon. TF32 matrix products deviate by $4.82$\,mm on this device and are slower than fp32, so the Tensor Core path is dominated on both axes. These rows are retained because they delimit the precision space in which the component-aware scheme of \Cref{subsec:mixed-precision} operates.

The remaining baselines are compared on the linear filtering core that they support. There the batched \texttt{torch-kf} implementation and the \textsc{PrefixScan} of S\"arkk\"a and Garc\'ia-Fern\'andez reproduce the sequential fp64 recursion to within $10^{-14}$ in fp64 and $3\times10^{-7}$ in fp32 at the final state, so their throughput differences do not come at the cost of accuracy; their precision on the nonlinear SINS/EKF workload is not reported because both methods assume a linear core. Finally, batch execution does not perturb numerical results: identical trajectories replicated across blocks agree bit-exactly (maximum cross-block spread $0$\,m), and independently seeded Monte Carlo trajectories at $B{=}108$ show an ensemble RMSE of $1008.9\pm97.0$\,mm in X, a genuine trajectory-to-trajectory spread rather than an implementation artifact.

\begin{table*}[!t]
  \centering
  \caption{Trajectory-level accuracy and throughput on the H20 GPU over the full $166{,}667$-step trajectory. $\Delta$ is the maximum axis-wise deviation from the fp64 CPU reference; the reference positioning error is $1{,}017.8$\,mm RMSE in X. Timings for \texttt{torch} eager and \texttt{torch.compile} over the full horizon are pending.}
  \label{tab:precision-fidelity}
  \begin{tabular}{@{}llrrrl@{}}
    \toprule
    Implementation & Precision & Steps/s & RMSE X (mm) & Max $\Delta$ (mm) & Status \\
    \midrule
    CPU eager (reference) & fp64 & 1,866 & 1,017.838 & --- & reference \\
    CUDA Graph & fp64 & 2,640 & 1,017.843 & 0.005 & bit-faithful \\
    \textsc{Trident} & fp64 & 16,491 & 1,017.842 & 0.128 & safe \\
    \textsc{Trident} & fp32 & 242,737 & 1,017.730 & 0.129 & adopted \\
    \textsc{Trident} & fp16 input & 249,785 & 1,017.841 & 2.188 & safe, no gain \\
    \textsc{Trident} & TF32 dots & 208,077 & 1,022.550 & 4.820 & reject \\
    \textsc{Trident} & bf16 input & 250,143 & 1,028.743 & 1,005.840 & reject \\
    \textsc{Trident} & fp16 state & 252,808 & 281,771.447 & diverged & reject \\
    \bottomrule
  \end{tabular}
\end{table*}
