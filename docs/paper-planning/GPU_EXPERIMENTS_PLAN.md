...
> 日期：2026-09-08 · 状态：**G1/G2/G3 已于 A100 执行完毕（2026-09-08 晚）**，产出
> `full_tensor_baselines_a100.json` / `linear_accuracy_a100.json` / `trident_precision_a100.json`，
> §5.3 已按单一来源（A100）重写回填；G4/G5/G6/G7 待后续 GPU 会话执行。
> 执行环境：A100-SXM4-80GB，torch 2.9.1+cu128 / triton 3.5.1（与论文 Table 5 A100 列已对齐）。
