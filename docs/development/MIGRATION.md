...
|--------------|-------------|------|
| `code/positioning/matlab/fangzhen.m` | `code/positioning/fangzhen.py` | 轨迹仿真数据生成 |
| `code/positioning/matlab/EKF.m` | `code/positioning/ekf.py` | SINS/EKF 融合定位 |

## 运行方式

```bash
cd RobotSim/
python code/positioning/fangzhen.py   # 生成 data/trajectory/PipeRobot_Trajectory.csv
python code/positioning/ekf.py        # 运行融合定位，输出 RMSE
```
...
| CSV文件名 | MATLAB 和 Python 统一写入 `data/trajectory/PipeRobot_Trajectory.csv` | 数据与代码分目录保存 |
...
pos_fusion, vel_fusion, pos_true, t = run_ekf('data/trajectory/PipeRobot_Trajectory.csv')
```
