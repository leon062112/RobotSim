...
> 数据集: `data/trajectory/PipeRobot_Trajectory.csv` (N=166667, 100Hz, ~500m)
...
| `data/results/` | 所有 benchmark/profiling 原始数据 |
...
```bash
# 单轨迹最优: v4 fp32
python code/optimization/ekf_v4.py  # 243k st/s, 亚mm

# 传感器 fp16 带宽最优: v6 fp16-ieee
python code/optimization/ekf_v6_per_component.py  # 230k st/s, +0.5mm
...
# 全量 benchmark
cd /root/RobotSim && python code/optimization/benchmark.py

# SM profiling
ncu --set full --kernel-name ekf_kernel_v6 \
    python code/optimization/profile_sm_v7.py v6 N=20000 combo=fp16-ieee
```
