...
> 数据集: `data/trajectory/PipeRobot_Trajectory.csv` (N=166667 步, 100Hz, 500m 管道)
...
(论文 Background 章节素材,数据见 `data/results/small_matrix_characterization.json`)
...
### 1.3 四重 GPU 不友好性 (量化)
ncu 实测单条轨迹 (`data/results/sm_profiling.json`):
...
(代码 `code/optimization/ekf_v4.py`,数据 `data/results/v4_precision_full.json`)
...
(代码 `code/optimization/ekf_v5.py`,数据 `data/results/v5_batch_scaling.json` + `data/results/sm_profiling.json`)
...
(数据 `data/results/data_pattern_analysis.json`)
