...
Two measurements:
  ...
  (B) Single-block occupancy.  Delegated to ncu (see run_ncu() below); the
      authoritative captured values live in data/results/sm_profiling.json.

Outputs:
  data/results/motivation_single_trajectory.json   (this run, measurement A)
  stdout summary suitable for transcribing into the paper.

Run:
  python code/optimization/motivation_single_trajectory.py            # wall-clock sweep only
  python code/optimization/motivation_single_trajectory.py --ncu      # also attempt ncu capture
"""
import os
import sys
import json
...
def main():
    ...
    with open('data/results/motivation_single_trajectory.json', 'w') as f:
        json.dump(out, f, indent=2)
    print('[motivation_3.3] wrote data/results/motivation_single_trajectory.json')
