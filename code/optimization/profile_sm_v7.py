...
用法:
  ...
  ncu --set full --kernel-name ekf_mega_batch_kernel \
      python code/optimization/profile_sm_v7.py v5 N=20000 B=78
  ...
  for B in 1 8 78 128 256 312; do
    ncu --set full --kernel-name ekf_mega_batch_kernel \
        -o profile_B${B} \
        python code/optimization/profile_sm_v7.py v5 N=20000 B=${B}
  done
"""
import sys
import os
...
def main():
    ...
    if mode == 'v4':
        ...
        import numpy as np
        data = np.loadtxt('data/trajectory/PipeRobot_Trajectory.csv', delimiter=',', skiprows=1)
